...
class ElasticNetModel(object):
    ...
    def _build_model(train_data, predictors):
        """Model: Elastic Net CV model using best estimator parameters"""
        model = ElasticNet(
            fit_intercept=True, normalize=True, warm_start=True,
            max_iter=100000, tol=1e-7, l1_ratio=.98, alpha=0.03,
            selection='random')
    ...
    def _make_predictions(self):
        ...
        model_coefficients.loc[0] = model.coef_
    ...
    def submit_solution(self):
        ...
        submission.to_csv(
            "../Submissions/submission_EN_best_estimator_" + str(
                self._calculate_evaluation_metric()) + ".csv", index=False)
        model_coefficients.to_csv(
            "../Model_results/EN_best_estimator_coefficients.csv", index=False)

ElasticNetModel().submit_solution()
